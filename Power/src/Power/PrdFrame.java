...
public class PrdFrame {
	...
	private void initialize() {
		...
		ImageIcon ico_fbdt = new ImageIcon(Login.class.getResource("fbdt.png")) ;//背景图案
		...
		label_NowUser.setFont(new Font("宋体", Font.PLAIN, 12));
...
//				insert into prend values('p020','u017','2020-05-07','为什么要回学校了？？？？？');
				...
				try {
					...
					st=conn.createStatement();
					st.execute(sqlStr);
					JOptionPane.showMessageDialog(null, "发布动态成功！","发布动态成功~",JOptionPane.INFORMATION_MESSAGE);
				}
				catch(Exception e1) {
					JOptionPane.showMessageDialog(null, "数据库连接失败"+e1.getMessage());
				}
